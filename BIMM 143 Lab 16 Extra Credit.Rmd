---
title: "BIMM 143 Lab 16 Extra Credit"
...
```{r}
metaFile <- "https://bioboot.github.io/bimm143_F21/class-material/GSE37704_metadata.csv"

countFile <- "https://bioboot.github.io/bimm143_F21/class-material/GSE37704_featurecounts.csv"
...
mycols[ inds ] <- "blue"
plot( res$log2FoldChange, -log(res$padj), col=mycols, xlab="Log2(FoldChange)", ylab="-Log(P-valu
e)" )
```
icolumns
```{r}
...
> Q6. Can you do the same procedure as above to plot the pathview figures for the top 5 down-reguled pathways?
...
pathview(gene.data=foldchanges, pathway.id=keggresids, species="hsa")
```
# 3. We can also do a similar procedure with gene ontology. Similar to above, go.sets.hs has all GO terms. go.subs.hs is a named list containing indexes for the BP, CC, and MF ontologies. Let’s focus on BP (a.k.a Biological Process) here
...
**endosomal/vascular pathway, listed to not match the previous KEGG results. KEGG uses the threshold p-value of 0.05 while this uses the most significant.**
